...
export default function InvitationAlert({ invitations }: InvitationAlertProps) {
  ...
  return (
    <div className="mb-8 space-y-4">
      {invitations.map((inv) => {
        const tipoLabel = inv.propuestaTipo === "investigacion" ? "Investigación" : inv.propuestaTipo === "proyecto" ? "Proyecto" : "Pasantía";
        return (
          <div
            key={inv.id}
          ...
          >
            <div className="flex gap-4">
              ...
              <div>
                <h3 className="font-bold text-lg text-white">Invitación a Equipo de {tipoLabel}</h3>
                <p className="text-sm text-red-50 mt-1">
                  <strong>{inv.invitadorNombre}</strong> ({inv.invitadorCarnet || 'Sin carnet'}) te ha invitado a unirte a su propuesta de tipo {tipoLabel}.
                </p>
              </div>
            </div>
